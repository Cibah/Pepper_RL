...
class RewardTrackerThread (threading.Thread):
    ...
    def detectRect( self, number, isRect) :
        ...
        if len(cnts) > 0:
            ...
            if (size[0] + size[1])> self.conf["size1_thresh" if number == 1 else "size2_thresh"] :
                    ...
                    box = np.int0(box)
                    cv2.drawContours(self.frame, [box], 0, self.color1 if number==1 else self.color2, 2) 

                    if (number == 1) :
                        cv2.putText(self.frame, str(center),
                                (self.frame.shape[1]-180, self.frame.shape[0]-10),
    ...
    def detectObject( self, number) :
        ...
        if len(cnts) > 0:
            ...
            if radius > self.conf["size1_thresh" if number == 1 else "size2_thresh"] :
                    ...
                    cv2.circle(self.frame, center, 5, self.color1 if number==1 else self.color2, -1)
                    if (number == 1) :
                        cv2.putText(self.frame, str(center),
                                (self.frame.shape[1]-180, self.frame.shape[0]-10),
                                cv2.FONT_HERSHEY_SIMPLEX, 1, (210,210,210), 8)
    ...
    def drawTrace( self, newPoint) :
        ...
        for i in range(1, len(self.pts)):
            ...
            thickness = int(np.sqrt(self.arguments["buffer"] / float(i + 1)) * 2.5)
            cv2.line(self.frame, self.pts[i - 1], self.pts[i], (0, 205, 205), thickness)
    ...
    def run(self) :
        ...
        while not self.event.is_set() :  
            ...
            self.frame = self.frame[1] if self.arguments.get("video", False) else self.frame
            ...
            # color space
            self.frame = imutils.resize(self.frame, width=self.conf["videoframe_width"] )
            ...
            if (center2 is not None ) and (center1 is not None):
                    ...
                    cv2.putText(self.frame, str(delta), 
                            (self.frame.shape[1]//2 -60, self.frame.shape[0]-10),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (210,210,210), 8)
                    cv2.putText(self.frame, str(delta), 
                            (self.frame.shape[1]//2 -60, self.frame.shape[0]-10),
            ...
            key = cv2.waitKey(1) & 0xFF             # required to generate video output
